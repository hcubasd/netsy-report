...
# latex
if ! command -v texlab; then
	curl -fL "https://github.com/latex-lsp/texlab/releases/download/v5.25.1/texlab-${MACHINE_ARCH}-linux.tar.gz" |
		tar -xz -C /usr/local/bin texlab
fi
if ! command -v tex-fmt; then
	curl -fL "https://github.com/WGUNDERWOOD/tex-fmt/releases/download/v0.5.7/tex-fmt-${MACHINE_ARCH}-linux.tar.gz" |
		tar -xz -C /usr/local/bin tex-fmt
fi
